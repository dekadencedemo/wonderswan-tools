...
pub fn is_mod_file(mod_bytes: &[u8]) -> bool {
...
pub fn read_mod(mod_bytes: &[u8]) -> Result<Song, &'static str> {
    let title_start = 0;
    let title_length = 20;
...
fn parse_patterns(mod_bytes: &[u8], max_pattern: u8) -> Vec<Pattern> {
    let mut patterns: Vec<Pattern> = Vec::new();

    for pattern_index in 0..=max_pattern {
        patterns.push(Pattern {
            rows: parse_pattern_rows(mod_bytes, pattern_index),
        });
    }
...
fn parse_pattern_rows(mod_bytes: &[u8], pattern_index: u8) -> Vec<PatternRow> {
    let mut rows: Vec<PatternRow> = Vec::new();
    let pattern_length = 64;

    for row_index in 0..pattern_length {
        rows.push(PatternRow {
            channel_rows: parse_mod_channel_rows(mod_bytes, pattern_index, row_index),
        })
    }
...
fn parse_mod_channel_rows(mod_bytes: &[u8], pattern_index: u8, row_index: u8) -> Vec<ChannelRow> {
...
fn parse_samples(mod_bytes: &[u8], max_pattern: u8) -> Vec<Sample> {
    ...
    let mut sample_data_position =
        (PATTERNS_START + (((max_pattern as u16) + 1) * PATTERN_LENGTH)) as usize;
    ...
    for _ in 0..31 {
        ...
        let repeat_length =
            u16::from_be_bytes([repeat_length_bytes[0], repeat_length_bytes[1]]) * 2;
